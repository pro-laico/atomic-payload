...
Why blocks? Each action is a plain Payload block: a slug plus the option fields an editor fills in (the theme toggle, for example, is a small block with a single select field). The plugin registers the built-ins once on your config, and atomic blocks reference that shared list wherever actions are allowed. The payoff is that editors compose behavior in the admin the same way they compose content, and the admin stays the source of truth for what runs, with no per-component JavaScript to write or keep in sync.
...
Actions are provided by the `actions` namespace of `@pro-laico/atomic` and registered with the `actionsPlugin` factory. There is little to configure: calling it with no options registers every built-in action block on your config, so the admin flow above works out of the box. The options it does take turn the plugin off (`enabled`) or contribute extra action blocks (`actionBlocks`):
