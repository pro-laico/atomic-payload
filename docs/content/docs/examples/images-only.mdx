...
- **`<ResponsiveImage>`** emits a plain `<img>` whose `srcset` is stepped by its `pixelStep` (default 50px, configurable) up to the source's intrinsic width. It's not `next/image`, and it works in server and client trees. The endpoint honors each requested width exactly (no server-side snapping), so every srcset entry keeps the requested aspect ratio.
- **Blur placeholders** are generated on upload by the plugin itself (Sharp shrink + blur into each doc's `blurDataUrl`); `<ResponsiveImage>` reads it and fades the image in over it on load. No extra plugin or peer.
...
4. The page renders each image at its natural ratio plus 16:9, 1:1, and 9:16, each cropped to the focal point on demand, with a blur-up.
...
request /api/img/:id?w&h&ar&fit&q&fmt
        ...
        │
<ResponsiveImage image={doc} aspectRatio="16:9" />
        └─ plain <img srcset=…sizes=…style="aspect-ratio"> + blur-up fade-in
```
...
| File | What's there |
| --- | --- |
| `src/payload.config.ts` | `buildConfig`: `imagesPlugin()` (built-in blur placeholders), SQLite, Sharp |
...
<Callout type="info">
  No `@pro-laico/atomic` here: the `ImageChild` Atomic block (the only part of `@pro-laico/images` that needs atomic) isn't used. Everything else (the collection, the transform endpoint, `ResponsiveImage`, the focal UI, and blur) works with just `@pro-laico/core`.
</Callout>
