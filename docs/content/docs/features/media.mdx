...
Use `ImageChild` when an editor places an image inside page content: the block points at an upload in `Images` and adds per-placement display options like an alt override, lazy or eager loading, and a stored size version. When you need an image in your own schema, reference the collection directly instead, with an `upload` field whose `relationTo` is `images` on your collection or global, the same pattern the bundled `FaviconField` uses for favicons.
