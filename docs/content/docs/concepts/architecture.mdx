...
Beyond the factory, each package also exports its **raw collections, hooks, fields, and components** as named imports, for advanced consumers who want to wire pieces in by hand rather than take the whole plugin. The `@pro-laico/tracking` package, for instance, exports `postHogGroup()` and `googleTagManagerGroup()` so you can compose your own `Tracking` global instead of registering the bundled one.
