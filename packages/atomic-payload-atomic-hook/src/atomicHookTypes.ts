import type { StoredAtomicActions } from '@pro-laico/atomic-payload-types/schema'

/** Narrow getter used by `createCssProcessor` (e.g. app `getCached` / `unstable_cache` wrapper). */
export type AtomicHookGetCached = (
  tag: 'atomic-classes' | 'header' | 'footer' | 'designSet' | 'shortcutSet',
  draft: boolean,
) => Promise<unknown>

/** Constructor for `@pro-laico/atomic-payload-actions/processor` (passed from the app to avoid pulling actions into this package's tsc graph). */
