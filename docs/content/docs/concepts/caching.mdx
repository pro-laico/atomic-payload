...
Cached reads only stay correct because Payload hooks bust the matching tags when documents change. `core` ships factories for each hook phase:

- **`createRevalidateCacheAfterChange(handlers)`** (recommended) builds an `afterChange` hook that dispatches to a per-slug handler after a document write commits, revalidating the tags that depend on it.
- **`createRevalidateCacheOnDelete(handlers)`** is the `afterDelete` counterpart, for revalidating when a document is removed.
- **`createRevalidateCache(handlers)`** is the `beforeChange` variant. Prefer the `afterChange` one for plain collections; see the warning below for why.

Each takes a `slug → handler` map, so you wire revalidation for exactly the collections you have:

```ts
import { createRevalidateCacheAfterChange, createRevalidateCacheOnDelete, revalidateTag } from '@pro-laico/core'

const revalidateCacheAfterChange = createRevalidateCacheAfterChange({
  pages: async ({ data }) => {
...
The slug-bound default hooks (exported from `@pro-laico/core` as `revalidateCacheCollectionAfterChange`, `revalidateCacheCollection`, and `revalidateCacheOnDelete`) stay bound to the conventional slug set, so existing imports keep working unchanged.

Each `@pro-laico/*` collection and global bakes these hooks into itself, so installing a plugin wires its own revalidation with nothing to configure. For a collection a third-party plugin registers (for example the form builder's `forms` / `form-submissions`), use `revalidationPlugin` from `@pro-laico/core` to attach the same hooks to the slugs you name. Add it after the plugin that registers them:
