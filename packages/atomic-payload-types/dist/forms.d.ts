...
import type { Config, FormSubmission, StoredAtomicForm, AtomicInputTypes, FormRateLimitBlocks, FormSanitationBlocks, FormValidationBlocks, InputSanitationBlocks, InputValidationBlocks, FormRateLimitBlockType, FormSanitationBlockType, FormValidationBlockType, InputValidationBlockType, InputSanitationBlockType } from './payload-types';
...
export type SubmitToPayload = {
    field: string;
    value: string;
}[];
export type AllFormBlockTypes = FormRateLimitBlockType | FormSanitationBlockType | FormValidationBlockType | InputSanitationBlockType | InputValidationBlockType;
export type AllFormBlocks = FormRateLimitBlocks[number] | FormSanitationBlocks[number] | FormValidationBlocks[number] | (InputSanitationBlocks[number] & {
    inputName: string;
}) | (InputValidationBlocks[number] & {
    inputName: string;
});
...
export type FormFunction<TArgs extends {
    block: AllFormBlocks;
} = {
    block: AllFormBlocks;
}> = (args: AFPBase & TArgs) => Promise<AFPBase>;
export type FormFunctionsMap = {
    [K in AllFormBlocks as K['blockType']]: FormFunction<{
        block: K;
    }>;
};
export {};
