...
- **Programmatic:** `import { runDownloadFonts } from '@pro-laico/fonts/scripts/downloadFonts'` then `await runDownloadFonts({ ... })`.
- **CLI (published):** `atomic-fonts-download` bin (run `pnpm build` in this package so `dist/` exists).
- **Starter template:** `"download:fonts": "pnpm exec tsx node_modules/@pro-laico/fonts/src/scripts/cli.ts"` (ships `src/` in the package tarball).
