...
- After enabling the plugin (`INCLUDE_SEED=true`), run `pnpm payload generate:importmap` so Payload registers `@pro-laico/seed/admin/beforeDashboard` in its import map.
