...
`createIconSelect` is not re-exported from the package root so importing `IconLabelPath` / `AtomicIcon` from `@pro-laico/atomic-payload-icons` does not pull admin code into client bundles.

Optional icon-name presets for seeding or docs: `@pro-laico/atomic-payload-icons/iconSet/defaults`.
