...
A minimal Atomic Payload template that exercises **`@pro-laico/fonts`** on its own. Build typefaces in the `Font` collection, choose the active four in the `fontSet` global, run the font download step, and the site serves them with `next/font/local`.
...
- `fontsPlugin({ includeFontSet: true, fontOriginalOptions, fontOptimizedOptions })` registers the visible **`Font`** typeface collection (plus the hidden **`fontOriginal`** raw archive and **`fontOptimized`** served WOFF2s) **and** the **`fontSet` global**, the active `sans` / `serif` / `mono` / `display` selection (the font analog of an active icon set).
- `POST /api/seed` uploads four bundled open-source fonts into `fontOriginal`, creates a one-weight `font` typeface for each (the save hook subsets each into a served `fontOptimized`), **and points the `fontSet` global at the typefaces**. It is auth-gated and idempotent, run from the admin dashboard, so you have fonts to pick without finding your own.
...
```plaintext
build a Font typeface (admin)  ──►  raw file → fontOriginal (private)
        │                            save hook subsets it → fontOptimized (served WOFF2)
seed: POST /api/seed uploads public/seed-fonts/*.woff2 → fontOriginal,
      creates a font typeface per role, then
      updateGlobal('fontSet', { sans, serif, mono, display })  ◄── active set
        │
pnpm generate:fonts ── GET /api/fonts/export @ FONT_DOWNLOAD_URL ──► public/fonts/* + src/app/definition.ts
        │   (export reads each active typeface's fontOptimized bytes)
...
The active selection lives in a **global** because it's the active-selection layer, mirroring icons-only's active `iconSet`. The `Font` collection can hold many typefaces; the `fontSet` global picks the one `sans` / `serif` / `mono` / `display` the site uses, editable at `/admin/globals/fontSet`.
...
The endpoint reads each active typeface's **served** (`fontOptimized`) font server-side, from disk for local storage, or by fetching the URL Payload reports for cloud adapters (Vercel Blob, S3, and the like), so there's no storage token or login to manage. When neither the URL nor the secret resolves, the download is skipped (the build still succeeds) and the specimens fall back to system fonts until you run it.
...
| File | What's there |
| --- | --- |
| `src/payload.config.ts` | `buildConfig`: `fontsPlugin({ includeFontSet: true })` with `fontOriginalOptions` / `fontOptimizedOptions` `staticDir`s |
...
| `src/app/(payload)/api/seed/route.ts` | `POST /api/seed` → upload originals, create typefaces, set the `fontSet` global |
| `src/app/(payload)/api/reset/route.ts` | `POST /api/reset` → clear the global + delete typefaces (cascades to originals/optimized) |
| `public/seed-fonts/*.woff2` | bundled OFL fonts (committed) + `LICENSES.md` |

<Callout type="info">
  `FONT_STATIC_DIR` (`./media`, gitignored) is where Payload writes uploads — the raw archive under `original/`, the served WOFF2s under `optimized/` — shared via `src/lib/fontDir.ts` so the config and the export read path never drift. The bundled seed fonts (Inter, Lora, JetBrains Mono, Abril Fatface) are OFL: see `public/seed-fonts/LICENSES.md`, and replace them before shipping a real project.
</Callout>
