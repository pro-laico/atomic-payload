...
export const cssProcessor: cssProcessorType = async ({ slug, context, draft, req }) => {
  const pagesAtomicClasses = await getCached({ tag: 'atomic-classes', draft })
  const header = slug !== 'header' ? await getCached({ tag: 'header', draft }) : context?.header
  const footer = slug !== 'footer' ? await getCached({ tag: 'footer', draft }) : context?.footer
  const ds = slug !== 'designSet' ? await getCached({ tag: 'designSet', draft }) : context?.designSet
  const shortcutSet = slug !== 'shortcutSet' ? await getCached({ tag: 'shortcutSet', draft }) : context?.shortcutSet

  // Classes contained within unoShortcuts are automatically added to the classNames set, but in createGenerator.
  const unoShortcuts = [...(shortcutSet?.shortcuts || []), ...(shortcutSet?.defaultShortcuts || [])].reduce(
