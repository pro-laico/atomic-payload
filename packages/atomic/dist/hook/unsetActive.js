...
export const unsetActive = async ({ id, draft, req, slug }) => {
    try {
        ...
        if (results && results.length > 0)
            manualLogger(`[UPDATE] Removed active status from: (${results?.join('), (')})`);
        else
            manualLogger(`[INFO] No active status to remove from ${slug} docs.`);
        return slug;
    }
    catch (error) {
...
//# sourceMappingURL=unsetActive.js.map