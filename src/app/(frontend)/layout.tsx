...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const css = await getCached({ tag: 'site-css', draft })
  const ds = await getCached({ tag: 'designSet', draft })
  const header = await getCached({ tag: 'header', draft })
  const footer = await getCached({ tag: 'footer', draft })
  const tracking = await getCached({ tag: 'tracking', draft })
  const storedAtomicActions = await getCached({ tag: 'atomic-actions', draft })
