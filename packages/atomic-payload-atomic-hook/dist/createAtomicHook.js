...
import { runAPF } from '@pro-laico/atomic-payload-apf'
...
export function createAtomicHook({ getCached, ActionBlockStorageProcessor }) {
  ...
  return async ({ collection, context, data, originalDoc, operation, req }) => {
    ...
    if (hasChildren) {
      traverse(sd).forEach(function (node) {
        if (r.classes && this?.key?.endsWith('ClassName') && node) node.split(/\s+/).forEach((cls) => classes.add(cls.trim()))
        if (
          r.forms &&
          this.path &&
          typeof node === 'object' &&
          ...
          if ('formSanitationBlocks' in node && node.formSanitationBlocks) formData.sanitation = node.formSanitationBlocks
          if ('formValidationBlocks' in node && node.formValidationBlocks) formData.validation = node.formValidationBlocks
          if ('formRateLimitBlocks' in node && node.formRateLimitBlocks) formData.rateLimiting = node.formRateLimitBlocks
          ...
          if (r.forms && this.path && typeof n === 'object' && 'type' in n && n?.type === 'input') {
            ...
            if (closestFormKey) {
              ...
              if (form) {
                ...
                if ('inputSanitationBlocks' in n && n.inputSanitationBlocks) inputData.sanitationBlocks = n.inputSanitationBlocks
                if ('inputValidationBlocks' in n && n.inputValidationBlocks) inputData.validationBlocks = n.inputValidationBlocks
    ...
    if (r.classes) {
      ...
      if (slug === 'designSet') processDesignSet(data)
      context[slug] = data
    }
    if (r.forms) {
