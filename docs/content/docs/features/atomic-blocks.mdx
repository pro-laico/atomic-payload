...
Atomic Blocks are provided by the `children` namespace of `@pro-laico/atomic`. Its `childBlocksPlugin` registers the `AtomicChild` block and the built-in `SimpleTextChild` block, forming the recursive tree:
...
Sibling packages contribute their own child blocks — for example `@pro-laico/images/blocks/imageChild` and `@pro-laico/icons/blocks/iconChild` — which get picked up automatically when those plugins are installed. So the set of blocks you can nest grows as you add integrations.
