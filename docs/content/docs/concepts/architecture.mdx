...
A full app just lists them in the `plugins` array, in order. The cross-cutting concerns are wired explicitly rather than by a magic finalizer: each plugin bakes its own cache-revalidation hooks into its collections and globals, and the shared `atomicHook` (which snapshots CSS, forms, and actions and enforces single-active sets) is handed to the plugins that own atomic content (`sitePlugin` bakes it on pages/header/footer, `stylesPlugin({ atomicHook })` on the design sets, and `iconsPlugin` on the icon set). For a collection a third-party plugin registers, `revalidationPlugin` from `@pro-laico/core` attaches the revalidation hooks to the slugs you name. The [atomic-payload template](/docs/templates/atomic-payload) wires its plugins this way.
