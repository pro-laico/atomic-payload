...
Atomic Payload is plugin-based, but the packages can't hard-code the types of the project that consumes them — every app generates its own `payload-types.ts` with its own block, collection, and global shapes. To stay decoupled, each domain package (`atomic`, `site`, `styles`, …) ships **schema stubs** written as `Get<'X', Default>` against the single `PayloadAugment` interface from `@pro-laico/core`.
