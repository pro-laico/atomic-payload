...
import type { Form, StoredAtomicForm } from '@pro-laico/atomic-payload-types/schema'
import cacheLogger from '../cacheLogger'

/** Gets all backend forms stored in the forms collection. */
export const getCachedBackendForms: GCFunction<'backend-forms'> = async (configPromise, tag) => {
...
export const getCachedAtomicForms: GCFunction<'atomic-forms'> = async (configPromise, tag, draft) => {
  const payload = await getPayload({ config: configPromise })
