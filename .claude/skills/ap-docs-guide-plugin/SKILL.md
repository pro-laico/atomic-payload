...
7. **## Options** — a `<TypeTable>` of the plugin options, with plain-language descriptions (say what the option does for the user, not its internal merge mechanics). **Document nested option values, not just the top level.** When an option's type is an object (or `false | object`) with its own keys, do NOT cram those keys into the parent's description prose (the common failure — most plugin pages stop at the first level and leave nested keys like `designSet: { access, collection, fontField }` undocumented). Instead give each nested key its own documented row (type, default, what it does) in a **separate `<TypeTable>`**, and **nest that table under its parent in the UI** by wrapping it in an `<Accordion title="optionName keys">` (the title is plain text — no backticks) placed directly below the main options table — collapsible and grouped under the parent, not flattened into a sibling section. Recurse for keys that are themselves objects. The reader should be able to expand a parent option and see every value it accepts. **End the Options section with one `ts` code block** showing the plugin invoked with every option at its default value — a copy-pasteable "here are the defaults" working example. Include nested option defaults, use the real default values (from the source), and mark required / no-default options with an inline comment (e.g. `generateLivePreviewPath, // required`).
...
- [ ] Object-typed options (`object` / `false | object`) have their nested keys documented as their own rows, nested under the parent in the UI (a parent-titled `<Accordion>` holding a nested `<TypeTable>`) — not buried in the parent's description.
- [ ] The Options section ends with one `ts` code block of the plugin invoked with every option (incl. nested) at its default value, as a working defaults reference.
