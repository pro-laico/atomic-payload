...
### Cache getters — `createDefaultGetRegistry` + per-getter factories

From `@pro-laico/core/cache`. `createDefaultGetRegistry({ pagesSlug, formsSlug })` builds the full registry of cached getters, and per-getter factories like `createGetCachedPages('articles')` let you configure a single getter.

```ts
import { createDefaultGetRegistry, createGetCachedPages } from '@pro-laico/core/cache'

const registry = createDefaultGetRegistry({ pagesSlug: 'articles', formsSlug: 'contactForms' })
const getCachedArticles = createGetCachedPages('articles')
```

### Admin fields — `createTestPathField` / `createLinkControlBarFields`

`createTestPathField(slug)` is from `@pro-laico/core`; `createLinkControlBarFields(slug)` is from `@pro-laico/atomic`. Each builds an admin field bound to the slug you pass.

```ts
import { createTestPathField } from '@pro-laico/core'
import { createLinkControlBarFields } from '@pro-laico/atomic'

const testPathField = createTestPathField('articles')
