...
describe('transform endpoint (integration)', () => {
  ...
  it('negotiates fmt=auto to webp by default (not avif) and sets Vary', async () => {
    const handler = getHandler(payload, 'get', '/img/:id')
  ...
  it('serves a burst of identical misses without creating duplicate variants', async () => {
    const handler = getHandler(payload, 'get', '/img/:id')
    const before = await countVariants(payload, sourceId)
    const q = 'w=512&fit=cover&fmt=webp' // distinct key (snaps to 500) — not generated by earlier tests
    const responses = await Promise.all(Array.from({ length: 6 }, () => handler(makeReq(payload, sourceId, q))))
    for (const r of responses) expect(r.status).toBe(200)
    await waitForVariants(payload, sourceId, before + 1)
    expect((await countVariants(payload, sourceId)) - before).toBe(1)
  })
