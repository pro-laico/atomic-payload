...
import { GCArgs, GCReturns, GCFunction } from '@/ts/types'
...
//KNOWN ISSUE: Needs a react cache wrapper.
...
async function getCached(args: GCArgs<'all'>): GCReturns<'all'> {
  const { tag } = args
  const tid = 'tid' in args ? args.tid : ''
  const draft = 'draft' in args ? args.draft : false
  const draftTag = 'draft' in args ? (draft ? 'draft' : undefined) : undefined
  ...
  if ('tid' in args) dependencyTags.push(mt([tag, args.tid, draftTag])) /* 'page:/example:draft' */
  ...
  switch (tag) {
    case 'icon': {
      ...
      const iconItem = args.iconSet?.iconsArray?.find((item) => item.name === tid)
