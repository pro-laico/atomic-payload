 ...
 */
export const revalidateCache = async ({ collection, data, originalDoc, context }) => {
    if (context.isSeed)
        return;
 ...
 * Uses collection slug to determine which revalidation logic to apply.
 */
export const revalidateCacheOnDelete = async ({ collection, doc }) => {
    ...
    switch (tag) {
        ...
        case 'designSet':
        case 'shortcutSet':
            if (Boolean(doc?.active))
                await revalidateTag(tag, false);
            break;
        case 'pages':
