...
import type { Runners, AllActions, Attributer, RunnerType, Attributers, ChildBlocks, ActionBlocks, ChildBlockType, AttributerType, ActionBlockType, AtomicInputTypes, AtomicButtonTypes, AtomicChildVariants, AtomicButtonPortalTypes } from './payload-types';
...
export type GetBaseAtt<T extends AttributerType | AttributerType[]> = T extends AttributerType[] ? {
    [K in keyof T]: Extract<Attributer, {
        type: T[K];
    }>;
} : Extract<Attributer, {
    type: T;
...
export type GetFullAtt<T extends AttributerType | AttributerType[]> = T extends AttributerType[] ? {
    actions: GetBaseAtt<T> | undefined;
    context: ActionContext;
...
export type GetBaseRun<T extends RunnerType | RunnerType[]> = T extends RunnerType[] ? {
    [K in keyof T]: Extract<Runners[number], {
        type: T[K];
    }>;
} : Extract<Runners[number], {
    type: T;
}>;
...
export type GetFullRun<T extends RunnerType | RunnerType[]> = T extends RunnerType[] ? {
    actions: GetBaseRun<T> | undefined;
    context: ActionContext;
...
export type ActionBlockMap = {
    [T in ActionBlocks[number] as T['blockType']]: T;
};
