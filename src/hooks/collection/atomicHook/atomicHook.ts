...
export const atomicHook: CollectionBeforeChangeHook = async ({ collection, context, data, originalDoc, operation, req }) => {
  ...
  if (slug === 'pages') {
    ...
    if (operation !== 'create' && previousHref !== href ? true : false) await revalidateTag({ tag: 'page', tid: previousHref, draft })
  }
  ...
  if (r.sitemap) await revalidateTag({ tag: 'sitemap', draft })
  if (r.forms) await revalidateTag({ tag: 'atomic-forms', draft })
  if (r.actions) await revalidateTag({ tag: 'atomic-actions', draft })
  if (r.siteMetadata) await revalidateTag({ tag: 'site-metadata', draft })
  if (slug !== 'pages' && r.active) await revalidateTag({ tag: slug, draft })
  if (slug === 'pages' && r.classes) await revalidateTag({ tag: 'atomic-classes', draft })

  if (r.pages) await revalidateTag({ tag: 'pages', draft })
  if (r.page && href) await revalidateTag({ tag: 'page', tid: href, draft })
